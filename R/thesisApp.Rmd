...
Base_Locations1 <- read.table("C:/Files/Documents/R/publicLibs/inst/extdata4/IDW/Straight.txt", header = TRUE, stringsAsFactors = F)
Base_Locations2 <- read.table("C:/Files/Documents/R/publicLibs/inst/extdata4/IDW/Drive.txt", header = TRUE, stringsAsFactors = F)

CONUS_Locations1 <- Base_Locations1[-c(2, 19, 21, 28),]
colnames(CONUS_Locations1) <- c("Base Name", "Short Name", "Abbr", "Long", "Lat", "5 Miles", "10 Miles", "15 Miles", "20 Miles",
...
CONUS_Locations2 <- Base_Locations2[-c(2, 19, 21, 28),]
colnames(CONUS_Locations2) <- c("Base Name", "Short Name", "Abbr", "Long", "Lat", "5 Miles", "10 Miles", "15 Miles", "20 Miles",
                               "25 Miles", "30 Miles", "35 Miles", "40 Miles", "45 Miles", "50 Miles", "55 Miles", "60 Miles",
                               "65 Miles", "70 Miles", "75 Miles", "80 Miles", "85 Miles", "90 Miles", "95 Miles", "100 Miles")
...
```{r}
  ...
  selectInput(inputId = "base", label = "Installation:",
              choices = CONUS_Locations1[,1], selected = NULL)
...
output$basemap <- renderLeaflet({
  ...
  base_lib <- read.table(paste(c(file.location, CONUS_Locations1[which(CONUS_Locations1[, 1] == inst), 3], "_Libs.txt"), collapse = ""), 
...
IDWData <- reactive({
  ...
  if(input$mode == "Straight Line Distance" ) {
    CONUS_Locations <- CONUS_Locations1
  }
    else {
      CONUS_Locations <- CONUS_Locations2
    }
...
```{r}
  ...
  selectedData <- reactive({
    ...
    if(input$mode == "Straight Line Distance" ) {
      ...
      # base_lib.sub <- base_lib.sub[, -"Distance"]
    }
